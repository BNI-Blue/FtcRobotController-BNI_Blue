...
public abstract class AutoMain extends LinearOpMode {
    ...
    public TeamPropPositionPipeline pipeline;
    public AprilTagProcessor aprilTag;
    public VisionPortal visionPortal;

    //Method to Initiliaze Camera
    public void initCamera() {
        int cameraMonitorViewId = hardwareMap.appContext.getResources().getIdentifier("cameraMonitorViewId", "id", hardwareMap.appContext.getPackageName());
        webcam =OpenCvCameraFactory.getInstance().
        createWebcam(hardwareMap.get(WebcamName.class, "Webcam 1"),cameraMonitorViewId);

        // Used for SamplePipe from WebcamExample. .
        pipeline =new

        TeamPropPositionPipeline();
    ...
    public static class TeamPropPositionPipeline extends OpenCvPipeline {
        ...
        private volatile TeamPropPositionPipeline.TeamPropPosition position = TeamPropPositionPipeline.TeamPropPosition.ONE;
        ...
        public Mat processFrame(Mat input) {
            ...
            if (maxArea == area1) {
                position = TeamPropPositionPipeline.TeamPropPosition.ONE;
                Imgproc.rectangle(input, region1_pointA, region1_pointB, GREEN, -1);
            }
            else if (maxArea == area2) {
                position = TeamPropPositionPipeline.TeamPropPosition.TWO;
                Imgproc.rectangle(input, region2_pointA, region2_pointB, GREEN, -1);
            }
            else {
                position = TeamPropPositionPipeline.TeamPropPosition.THREE;
                Imgproc.rectangle(input, region3_pointA, region3_pointB, GREEN, -1);
            }
            return input;
        }

        public TeamPropPositionPipeline.TeamPropPosition getAnalysis()
        {
            return position;
        }
